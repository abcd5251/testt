...
def ml_loop():
    ...
    while True:
        ...
        if vectors_y!=0:
         for j in range(len(scene_info.bricks)):
            if scene_info.bricks[j][1]-ball_y>0:
                if tempbrick_x+((scene_info.bricks[j][1]-tempbrick_y)/vectors_y)*tempvectors_x!=scene_info.bricks[j][0]:
                   if ball_willbe_x<0:
        ...
        if vectors_y!=0:
         if len(scene_info.hard_bricks)!=0:
            for j in range(len(scene_info.hard_bricks)):
                if scene_info.hard_bricks[j][1]-ball_y>0:
                  if tempbrick_x+((scene_info.hard_bricks[j][1]-tempbrick_y)/vectors_y)*tempvectors_x!=scene_info.hard_bricks[j][0]:
                    if ball_willbe_x<0:
                       temp=0-ball_willbe_x
                       ball_willbe_x=temp
